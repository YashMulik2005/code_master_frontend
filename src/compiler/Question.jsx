...
function Question() {
  ...
  const getdata = async () => {
    ...
    const result = await axios.post(`${url}/practice/question/${id}`, {
      data: data,
    });
    ...
    <div>
      {loader ? (
        <section className=" flex justify-center items-center w-[100%] h-[100vh]">
          <BarLoader size={80} color="green" />
        </section>
      ) : (
        <div>
          <div className=" flex items-center border-b-2 p-3 py-3 relative">
            <h1 className=" text-lg sm:text-2xl sm:mx-3 font-bold">
              {data.name}
            </h1>
            {theme == "light" ? (
              <MdDarkMode
                size={26}
                className="mx-2 sm:mx-3 cursor-pointer"
                onClick={handletheme}
              />
            ) : (
              <MdOutlineLightMode
            ...
            <h1 className="hidden sm:block sm:mx-3 font-semibold">
              {data.topic}
            </h1>
