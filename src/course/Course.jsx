                    ...
                    <div className=' grid grid-cols-1 w-[100%] md:grid-cols-2 md:w-2/3 gap-3 mt-2 sm:mt-0 max-[768px]:place-items-center'>
                        {
                            data?.map((item, index) => {
                                return <div className={` group hover:bg-[#39A84B]  ${theme == 'light' ? "bg-[#f5f1f0] " : " bg-[#0c131d] border-none"}  border-[1px] rounded-lg p-3 w-[100%] m-2 `} key={index}>
                                    ...
                                    {/* <h1>{item.id}</h1> */}
                                    {
                                        (item._id == track[item._id] ? <button className={` group-hover:bg-white cursor-pointer group-hover:text-black ${theme == "dark" ? "border-none bg-gray-900  " : ""} my-[5px] font-bold rounded-3xl border-[1px] px-3 sm:px-5 py-[2px] border-gray-400 hover:bg-[#39a84b] hover:text-white `}><Link to={`/course/${item._id}`}>continue</Link></button> : <button className={` group-hover:bg-white cursor-pointer group-hover:text-black ${theme == "dark" ? "border-none bg-gray-900  " : ""} my-[5px] font-bold rounded-3xl border-[1px] px-3 sm:px-5 py-[2px] border-gray-400 hover:bg-[#39a84b] hover:text-white `}><Link to={`/course/${item._id}`}>start</Link></button>)
                                    }
