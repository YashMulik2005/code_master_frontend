  ...
  return (
    <div className=" overflow-hidden">
      <div className=" max-[666px]:flex justify-between items-center p-2 bg-[#c1f8ca] hidden">
        ...
        >
          {navbar ? (
            <GiCancel size={33} className=" text-black" />
          ) : (
            <GiHamburgerMenu size={33} className=" text-black" />
          )}
        </section>
      </div>
      <div
        className={` ${
          theme == "light" ? "bg-white" : "bg-[#1c232b]"
        } h-[93vh] ${
          navbar ? "w-full sm:w-72" : "w-0"
        } absolute right-0 transition-all ease-linear duration-300 overflow-hidden z-10 ${
          theme == "light" ? "" : ""
        }`}
      >
        <ul className=" p-3 flex flex-col">
          <li
            className=" inline font-semibold hover:border-b-2 "
            onClick={() => {
              setnavbar(false);
              navigate("/");
            }}
        ...
        {logedin ? (
          ...
          <section>
            <button
              className=" mx-1 font-bold  rounded-3xl border-[1px] px-3 sm:px-5 py-[4px] border-gray-400 hover:bg-[#39a84b] hover:border-none hover:text-white"
              onClick={() => {
                setnavbar(false);
                navigate("/auth/login");
              }}
